...
**This table is generated** from `m3_exit_criteria.yaml` by `fs-exit-criteria --render`, and
`make governance` fails when it is stale or when a criterion's evidence does not resolve. Rows
marked **author-asserted** are the ones no machine stands behind: a judgement criterion may never
be auto-marked met, because that would lend a machine's authority to a claim no machine made.

**Why it is generated — written here because the table is where it would mislead.** Two
...
<!-- Generated by `fs-exit-criteria render`. 15 criteria. -->

| Criterion | Status | Evidence |
|---|---|---|
| E1 — every fold, split, sample and encoding groups by the feature's unit of history | **met** | test: `test_the_encoding_folds_by_account_not_by_row`, `test_the_categorical_encoding_folds_by_account_not_by_row` — restated per history_key after measurement refuted component folding for three of four relational keys |
| E2 — every reported metric states the scale it was measured at | **author-asserted** — author | a rule applied throughout rather than a checkable artefact; fs-features auc prints the corpus size, scored-row count and fraud count before any figure, but nothing prevents a future figure being quoted without its scale |
| E3 — the 44 engineered features re-checked against the D-08 0.80 ceiling | **NOT MET** | artifact: `docs/benchmarks/m3_single_feature_auc_2c80ef6.txt` at `2c80ef6` — re-check performed; five features exceed the ceiling, strongest 0.894. Resolved by the reporting gate (PB-46), not by moving the line |
| E4 — feature computation deterministic and byte-identical across batch sizes | **met** | test: `test_e4_the_vector_is_byte_identical_across_batch_sizes`, `test_e4_the_vector_is_deterministic_across_repeated_runs` |
| E5 — no feature reads a column the anti-leakage gates exclude | **met** | test: `test_e5_no_feature_reads_identifier_bytes`, `test_e5_no_feature_reads_row_position`, `test_e5_the_sub_second_part_of_a_timestamp_is_not_a_signal`, `test_e5_no_feature_reads_the_label_delay`, `test_e5_the_outcome_record_carries_no_confirmation_time` |
| E6 — the account_events join is available to features and measured alongside them | **met** | artifact: `docs/benchmarks/m3_single_feature_auc_2c80ef6.txt` at `2c80ef6` — days_since_sim_swap separates at 0.577 +/-0.177 on 11 fraud of 2,193 usable rows |
| E7 — PB-26 fixed before any feature reads the month partitions | **met** | test: `test_the_partition_key_is_the_simulation_month_and_its_drift_is_bounded` |
| E8 — country packs and corridor_class land in this branch, nothing hard-coded | **met** | test: `test_an_invented_country_in_an_invented_bloc_needs_no_code_change`, `test_no_country_currency_bloc_or_continent_is_named_in_the_feature_paths` |
| E9 — every new parameter carries provenance | **met** | gate: `uv run fs-dataset provenance --check` |
| E10 — the register covers every M3 requirement and the matrix is current | **met** | gate: `uv run fs-traceability check` |
| E11 — a milestone review covering leakage, determinism, grouping and claim backing | **met** | artifact: `docs/reviews/M3/milestone-review.md` at `2c80ef6` — one pass, BLOCKER and MAJOR only; two MAJOR found and both fixed in place |
| E12 — every bound, absence or invariant test asserts its own precondition first | **author-asserted** — author | applied throughout and visible in the suite, but no check can tell a precondition that constrains the fixture from one phrased to match it — which happened twice this milestone |
| E13 — the 44 feature tests each exercise their feature non-trivially | **author-asserted** — author | hand-computed cases with non-zero history, a crossed boundary or a structural NaN exist for every feature; "non-trivially" is not mechanically decidable |
| E14 — mutation checks run as the tests are written, and are recorded | **met** | test: `test_mutation_5_a_structural_missing_emitted_as_zero_is_detected`, `test_mutation_11_joining_current_thresholds_instead_of_as_of_ones_is_detected`, `test_mutation_14_the_speed_cap_applied_on_one_path_only_is_detected` — twelve of thirteen rows executed; row 9 needs the DB fallback path (PB-36) |
| E15 — no test mutates anything outside tmp_path, and a check enforces it | **met** | test: `test_the_geography_mutation_that_broke_a_real_run_is_reported`, `test_a_created_pack_is_reported_because_the_loader_globs_the_directory` |
