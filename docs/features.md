...
`denominations_by_currency`, so the feature is declared `COMPUTABLE`.
...
1. **Train on the features that carry information, and report that number with every metric.**
   ...
   went from 36 to 37, and PB-40 took it to **38** the next day when the generator began sharing
   devices. A figure measured on 36 stays a figure measured on 36.
...
3. **`accounts_per_device_7d` and `synthetic_identity_score` are live as of 2026-09-21.** The
   generator shares devices between accounts now (PB-40): a ring of synthetic identities transacts
   from one handset, and a share of ordinary customers use a handed-down one so that a shared
   device is not by itself a fraud signal. Measured on the regenerated benchmark, the feature takes
   **7 distinct values** where it used to take one, and the composite's fourth term fires.
