...
1. **M0 gate evidence.** The CI job `stack` (`make up` with `--wait`, then `make smoke`) is the
...
2. **Codespaces.** `.devcontainer/devcontainer.json` (4 cores / 16 GB; Docker-in-Docker; Temurin
   ...
   GitHub runner and re-runs the smoke test inside it, so the Codespaces setup itself is tested.
3. **Docker-dependent suites.** They run in CI on every push (`REQUIRE_DOCKER=1` for all CI jobs)
...
4. **Performance benchmarks** (M5, M6, M10 and every row in
   ...
   (`machine: <id>`). `fs-traceability check` enforces this: benchmark rows need a measurement
   file under `docs/benchmarks/`, and a CI run URL alone is not accepted as their evidence. CI may
   still run benchmarks as smoke tests (does it run, does it regress grossly), labelled as such.
5. **Memory budgets per compose profile** (MiB, sum of `deploy.resources.limits.memory`):
   ...
   a limit, uses a profile without a budget, or a profile exceeds its budget.
...
- Benchmark rows cannot be closed as `DONE` until a dedicated machine is provisioned and recorded;
  this is intentional.
