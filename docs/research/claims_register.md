...
| C-6 | "Ensemble reduces variance 12% vs single model" (2.3) | MEASURED, MIXED | Over 5 fixed seeds (1-5), with E1's time-ordered encoding: ensemble stdev 0.000612, vs XGBoost alone 0.000609 (-0.6%, did not reduce) and LightGBM alone 0.000700 (+12.5%, reduced). The claimed 12% does not hold against XGBoost; against LightGBM it now happens to match, which is a coincidence of this draw rather than support for the claim. The first measurement (-0.5% / +62.9%, `docs/benchmarks/m4_seed_variance_pb61.txt`) used the random-fold encoding E1 forbids, which made LightGBM's AUC far more seed-sensitive; the restated figures are `docs/benchmarks/m4_seed_variance_d8083dbc_e1.txt`. | M4 |
