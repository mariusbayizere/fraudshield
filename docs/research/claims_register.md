...
| C-9 | "No single feature separates the classes beyond AUC 0.80" (D-08 control, quoted in the datasheet, walkthrough and defence questions) | MEASURED | `uv run fs-dataset report <dataset> --rows 1000000 --seed 20260917` at 1,006,249 rows, commit `88f73b7`. Strongest transaction column `merchant_category_code` **0.711**; strongest channel overall, via the `account_events` join, event-to-transaction delay **0.758**; event type 0.537. All inside the 0.80 ceiling. Raw output `dataset/realism_report.md` | M2 |
