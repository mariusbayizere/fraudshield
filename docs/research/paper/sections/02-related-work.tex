...
and that no reference is invented; every citation below is \cite{TODO-verify} until the author
has read and verified the source. No claim about what a cited work reports is made here.
...
\cite{TODO-verify}, as is the treatment of class imbalance by reweighting rather than resampling
\cite{TODO-verify}. Our method follows this line (Section~\ref{sec:method}) and does not claim
novelty in it.

\paragraph{Mobile money and East African payments.} Work on fraud in mobile money ecosystems,
SIM-swap takeover, and agent networks \cite{TODO-verify}; regulatory and industry reports on
payment volumes in the region \cite{TODO-verify}. Where this paper uses a regional statistic it
...
mobile money transactions, and public synthetic fraud datasets \cite{TODO-verify}. The question
...
temporal cross-validation as defences \cite{TODO-verify}. Several of our failure modes
(Section~\ref{sec:discussion}) are instances of these.

\paragraph{Calibration and explanation.} Isotonic and Platt calibration \cite{TODO-verify};
expected calibration error and its dependence on binning \cite{TODO-verify}; exact TreeSHAP for
tree ensembles and its additivity property \cite{TODO-verify}. We use these methods as specified
...
DeLong's test for correlated AUCs, bootstrap and Wilson intervals \cite{TODO-verify}; temporal
hold-outs and leave-one-group-out designs for distribution shift \cite{TODO-verify}.
