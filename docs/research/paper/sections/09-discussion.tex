...
history is a counterparty or a map cell. The D-08 ceiling was true of the columns it measured and
false of the features it was quoted about. The fingerprint fix itself first hashed only some of
...
\paragraph{The wrong denominator.} The central methodological error of M2 was standardising a
detector's offset against a theoretical null rather than the observed seed-to-seed spread; a
single three-sigma reading turned out to be noise once repeated seeds were run. The first model
margin in M4 subtracted a feature measured on a few hundred rows holding two frauds from a model
...
after they had been analysed and written up, by the same author in the same session. The lesson
...
\subsection{What a builder of synthetic benchmarks should take from this}

Run, before trusting a synthetic benchmark: the best single feature on the engineered features, not
only on the raw columns; a leave-one-group-out \emph{and} a one-group-only ablation, since the first
alone cannot distinguish redundancy from irrelevance; a check that every generalisation experiment
withholds a mechanism and not just a label; and at least one pre-registered variant built from an
independently documented typology. On FraudShield-EAC each of these changed what could be claimed.
