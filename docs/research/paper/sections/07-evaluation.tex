...
\nTestFraud{} confirmed fraud), for models trained on \nTrainUsed{} rows, and are provisional
until the M4 milestone is tagged (see the note on the first page).
...
(\texttt{velocity\_ratio\_1h\_vs\_30d}) on the same rows and the margin over it. Source:
\texttt{docs/benchmarks/m4\_gate\_d8083dbc\_v3.txt}.}
\label{tab:gate}
\begin{tabular}{lrrr}
\toprule
...
15 equal-mass bins is \nGateECE{}. In all, \nGatePass{} gate metrics pass. The two failures are
recorded, not tuned away; training on a fraction of the available rows is a candidate cause, not
a finding. At the flag threshold precision is \nGatePrecSix{}, so the operating point trades recall
for very few false alerts.
...
\begin{table}[h]
\centering
\caption{Baselines on the same test rows: AUC, recall at \nFPRBudget{} FPR, and DeLong $p$
...
\begin{table}[h]
\centering
\caption{Diagnostic model with one feature group only (draw \texttt{d8083dbc}, source
...
the production ensemble restricted to card-style features loses \nAblCardD{} AUC, and on a
...
draw \texttt{6abde44e}, source \texttt{docs/benchmarks/m4\_frontier\_6abde44e.txt}). Milliseconds
...
generalised. The latency half is a property of the model and the explanation algorithm, which is
why an easy benchmark cannot weaken it: on data like this, the explanation budget, not the
accuracy target, should choose the tree configuration.
