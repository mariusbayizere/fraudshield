...
label-free synthetic-identity composite. The composite is the mean of four terms (low KYC tier, a
new account, device sharing and a volume ramp), but this benchmark has no KYC tier or account
opening date, so two of its terms are always zero and it reads device sharing and the volume ramp
only (Appendix~\ref{app:features}). Every label-derived feature uses only labels whose
availability time precedes the transaction. The catalogue in the repository is generated from the
code's feature registry, so documentation cannot drift from implementation; Appendix~\ref{app:features}
lists every feature by group.
...
and fails for the engineered features.
...
\begin{table}[htbp]
\centering
\caption{Single features above the separation ceiling (\nCeiling{}), $\max(\mathrm{AUC}, 1-\mathrm{AUC})$, draw
\texttt{6abde44e} (\nFloorDrawRows{} rows), \nScoredRows{} scored rows holding \nScoredFraud{} fraud,
...
a generator whose fraud was not would be the weaker artefact; and we did not restate the control as a
