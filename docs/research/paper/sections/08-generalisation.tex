...
A generalisation test needs a mechanism that actually differs, and one is documented: the
``sent by mistake'' reversal scam among Kenyan M-PESA users \cite{dogan2025grift}. In its common
documented form the victim receives a forged M-PESA confirmation message, so no money has in fact
arrived, and is persuaded to return the ``mistaken'' amount with a single transfer, usually to a
counterparty the victim has never paid before.
...
modelling choice, not a property of the documented scam (ADR 0028). It removes both axes this
...
\begin{figure}[htbp]
\centering
\input{figures/fig-variants.tex}
