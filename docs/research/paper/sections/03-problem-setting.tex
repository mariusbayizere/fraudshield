...
\paragraph{Corridors.} Transfers within a country, within the East African Community (EAC), and
to countries outside it are different populations: regional remittances, including cross-border
salary and family transfers, are routine and must not be treated like arbitrary foreign
transfers. A categorical \texttt{corridor\_class} feature separates the three.
...
for flagged transactions (build prompt Part C.2). A decision that holds a transaction for an
