...
| `razaq2021scams` | Crossref 10.1145/3449115 (title and subtitle, PACM HCI 5(CSCW1), pp. 1-30); the paper's own abstract, as the publisher deposited it with Crossref and as Semantic Scholar holds it | Verified. The abstract states "We interviewed 96 participants... 71 victims of mobile-based scams, seven non-victims, 15 mobile money agents, and three officials", and SMS- and call-based fraud in Pakistan, which is what the text cites it for. **The body is closed** (ACM DL returns 403; OpenAlex: closed, no open copy), so the count is confirmed from the abstract, not from the body |
