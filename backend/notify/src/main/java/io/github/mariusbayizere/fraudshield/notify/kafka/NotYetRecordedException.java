 ...
 * EnvelopeConsumer} re-reads such a record, with backoff, until it is {@link
 * EnvelopeConsumer#PARENT_WAIT} old, and then dead-letters it (reason {@code parent_not_recorded}).
 * A partition then never stalls behind a parent that will never arrive.
 */
